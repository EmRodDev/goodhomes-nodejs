...
const emailRegistration = async (data) => {
    ...
    await transport.sendMail({
        ...
        html: `
        <p>Hi there, ${name}</p>
        <p>Your account at goodhomes.com was created successfully, but it requires to be activated in order to use it.</p>
        <p>Please enter the following link to do that: 
        <a href="${process.env.BACKEND_URL}:${process.env.BACKEND_PORT ?? 3000}/auth/confirm/${token}">Confirm account</a>
        </p>
...
const emailForgotPassword = async (data) => {
    ...
    await transport.sendMail({
        ...
        <a href="${process.env.BACKEND_URL}:${process.env.BACKEND_PORT ?? 3000}/auth/forgot-password/${token}">Reset your password</a>
        </p>
